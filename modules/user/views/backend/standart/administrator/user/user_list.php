							...
							<div class="widget-user-header">
								<div class="row pull-right">
									...
									<a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', cclang('user')); ?> (Ctrl+a)" href="<?= site_url('administrator/user/add'); ?>">
										<i class="fa fa-plus-square-o"></i> <?= cclang('add_new_button', cclang('user')); ?></a>
									<?php }) ?>
									...
									<a class="btn btn-flat btn-success" title="<?= cclang('export', 'Excel '.cclang('user')); ?>" href="<?= site_url('administrator/user/export'); ?>">
										<i class="fa fa-file-excel-o"></i> <?= cclang('export', 'Excel '.cclang('user')); ?></a>
									<?php }) ?>
									<?php is_allowed('user_export', function(){?>
									<a class="btn btn-flat btn-success" title="<?= cclang('export', 'PDF User'); ?>" href="<?= site_url('administrator/user/export_pdf'); ?>">
										<i class="fa fa-file-pdf-o"></i> <?= cclang('export', 'PDF'); ?></a>
									<?php }) ?>
								</div>
									...
									<tbody id="tbody_user">
										...
										<tr>
											...
											<td>
												<div class="chip">
											<?php
												if (is_file(FCPATH . 'uploads/user/' . $user->avatar)) {
													$img_url = BASE_URL . 'uploads/user/' . $user->avatar;
												} else{
											...
											<td><?= _ent($user->bidang_id != null ? _ent($user->bidang_nama) : '-'); ?></td>
											<td><?= _ent($user->username); ?></td>
											<td><?= _ent($user->email); ?></td>
											<td><?= join_multi_select(implode(',', $this->model_user->get_group_user($user->id)), 'aauth_groups', 'id', 'name');?></td>
											<td><input type="checkbox" name="status" data-user-id="<?= $user->id; ?>" class="switch-button" <?= $user->banned ?:'checked'; ?>></td>
											<td width="200">
												<?php is_allowed('user_view', function() use ($user){?>
												<a href="<?= site_url('administrator/user/view/' . $user->id); ?>" class="label-default">
													<i class="fa  fa-newspaper-o"></i> <?= cclang('view_button'); ?></a>
												<?php }) ?>
												<?php is_allowed('user_update', function() use ($user){?>
												<a href="<?= site_url('administrator/user/edit/' . $user->id); ?>" class="label-default">
													<i class="fa fa-edit"></i> <?= cclang('update_button'); ?></a>
												<?php }) ?>
												<?php is_allowed('user_delete', function() use ($user){?>
												<a href="javascript:void(0);" data-href="<?= site_url('administrator/user/delete/' . $user->id); ?>" class="label-default remove-data">
