...
*|
*/
class Laporan_mutasi extends Admin {
	public function __construct() {
		parent::__construct();

		$this->load->model('model_laporan_mutasi');
	...
	public function stok_keluar_masuk($get_kategori, $get_bidang, $get_periode, $get_bulan, $get_tahun, $get_tgl_awal, $get_tgl_akhir) {
		...
		$query_stok_awal 	= $this->model_laporan_mutasi->query_stok_awal($tanggal_awal_bulan, $tanggal_akhir_bulan, $get_kategori, $get_bidang)->result();
		$query_stok_akhir 	= $this->model_laporan_mutasi->query_stok_akhir($tanggal_awal_bulan, $tanggal_akhir_bulan, $get_kategori, $get_bidang)->result();
	...
	*/
	public function index($offset = 0) {
		$this->is_allowed('laporan_mutasi_list');
		...
		$this->data['laporan_mutasi'] 			= $data;
		$this->data['laporan_mutasi_counts'] 	= count($data);

		$config = [
			'base_url'     => 'administrator/laporan_mutasi/index/',
			'total_rows'   => $this->data['laporan_mutasi_counts'],
		...
		$this->template->title('Laporan Mutasi List');
		$this->render('backend/standart/administrator/laporan_mutasi/laporan_mutasi_list', $this->data);
	}
	...
	*/
	public function export_excel() {
		$this->is_allowed('laporan_mutasi_excel');
		...
		foreach (db_get_all_data('barang', $conditions) as $item) {
			$data[$item->barang_id] = [
				...
				'stok_awal' 		=> $stok_keluar_masuk['stok_awal'][$item->barang_id],
				'mutasi_masuk' 		=> $stok_keluar_masuk['mutasi_masuk'][$item->barang_id],
				'mutasi_keluar' 	=> $stok_keluar_masuk['mutasi_keluar'][$item->barang_id],
				'stok_akhir' 		=> ($stok_keluar_masuk['mutasi_masuk'][$item->barang_id] - $stok_keluar_masuk['mutasi_keluar'][$item->barang_id]),
			];
		}

		$fields = ['no', 'nama_barang', 'kategori_barang', 'uom', 'stok_awal', 'mutasi_masuk', 'mutasi_keluar', 'stok_akhir'];
		...
		$this->excel->getActiveSheet()->setTitle(ucwords('Rekap Data Mutasi Barang'));

		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename=' . ucwords('Laporan Mutasi Barang') . '-' . date('Y-m-d') . '.xls');
